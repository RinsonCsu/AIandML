...
def create_shopping_cart() -> ShoppingCart:
    ...
    while True:
        ...
        if not customer_name.strip():
            aligned_print_withline("Cannot have blank or empty Customer Name. Please try again!")
        elif customer_name.strip() == "none":
            aligned_print_withline("Cannot have none as Customer Name. Please try again!")
        else:
    ...
    while True:
        ...
        if not current_date.strip():
            aligned_print_withline("Cannot have blank or empty Current Date. Please try again!")
        else:
...
def add_item_to_cart(shopping_cart: ShoppingCart):
    ...
    while True:
        ...
        if not item_name.strip():
            aligned_print_withline("Cannot have blank or empty Item Name. Please try again!")
        else:
    ...
    while True:
        try:
            ...
            aligned_print_withline("Invalid Item Descruotion. Please try again!")
            
    while True:
        try:
            item_price = float(aligned_input('Enter the item price:\n'))
            if item_price <= 0:
                aligned_print_withline("Item Price Cannot be negative or zero")
                continue
            else:
                item.item_price = item_price
                break
        except Exception as err:
            aligned_print_withline("Invalid Item Price. Please try again!")
            
    while True:
        try:
            item_quantity = int(aligned_input('Enter the item quantity:\n'))
            if item_quantity <= 0:
                aligned_print_withline("Item Quantity Cannot be negative or zero")
                continue
            else:
                item.item_quantity = item_quantity
                break
        except Exception as err:
            aligned_print_withline("Invalid Item Quantity. Please try again!")          

    shopping_cart.add_item(item)

def remove_item_from_cart(shopping_cart: ShoppingCart):
    if shopping_cart.get_num_items_in_cart() == 0:
        aligned_print_withline("Shopping cart is empty! Nothing to remove")
    else:
...
def modify_item_quantity(shopping_cart: ShoppingCart):
    if shopping_cart.get_num_items_in_cart() == 0:
        aligned_print_withline("Shopping cart is empty! Nothing to modify")
        return
